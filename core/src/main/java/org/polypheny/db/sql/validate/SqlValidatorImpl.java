...
import static org.polypheny.db.sql.SqlUtil.stripAs;
...
import org.polypheny.db.sql.SqlAccessEnum;
...
public class SqlValidatorImpl implements SqlValidatorWithHints {
    ...
    void lookupSelectHints( SqlSelect select, SqlParserPos pos, Collection<SqlMoniker> hintList ) {
        ...
        if ( (info == null) || (info.scope == null) ) {
            ...
            lookupNameCompletionHints( info.scope, info.id.names, info.id.getParserPosition(), hintList );
        }
    }
    ...
    private void registerOperandSubQueries( SqlValidatorScope parentScope, SqlCall call, int operandOrdinal ) {
        ...
        if ( operand.getKind().belongsTo( SqlKind.QUERY ) && call.getOperator().argumentMustBeScalar( operandOrdinal ) ) {
    ...
    private Table findTable( String alias ) {
        ...
        PolyphenyDbSchema.TableEntry entry = SqlValidatorUtil.getTableEntry( catalogReader, names );
    ...
    private void validateOrderItem( SqlSelect select, SqlNode orderItem ) {
        switch ( orderItem.getKind() ) {
            case DESCENDING:
                validateFeature( RESOURCE.sQLConformance_OrderByDesc(), orderItem.getParserPosition() );
    ...
    private static class Expander extends SqlScopedShuttle {
        ...
        protected SqlNode expandDynamicStar( SqlIdentifier id, SqlIdentifier fqId ) {
            if ( DynamicRecordType.isDynamicStarColName( Util.last( fqId.names ) ) && !DynamicRecordType.isDynamicStarColName( Util.last( id.names ) ) ) {
                ...
                inputs[0] = fqId;
                inputs[1] = SqlLiteral.createCharString( Util.last( id.names ), id.getParserPosition() );
